...
export default function Finland() {
  ...
  // Fetch top 10 cities by unique customers for Finland (country_id=72) on mount or when base changes
  useEffect(() => {
    fetch(`${base}/api/country/72/top-cities?limit=10`)
      ...
      .catch(e => {
        ...
        console.error("Error fetching Finland top cities by customers:", e);
      });
  }, [base]);

  // Fetch top 10 cities by revenue for Finland only when switching to "revenue" mode and not already loaded
  useEffect(() => {
    ...
    fetch(`${base}/api/cities_by_revenue`)
      ...
      .then(j => {
        ...
        // Finland's data: array of { city, ksek }
        const finland = Array.isArray(map?.["Finland"]) ? map["Finland"] : [];
        setRowsRevenue(finland);
      })
      .catch(e => {
        // Log fetch or parse errors for debugging
        console.error("Error fetching Finland top cities by revenue:", e);
      });
  ...
  return (
    <main style={{ padding: 20, maxWidth: 1000, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 12 }}>
        <h2>Finland · Top 10 cities by {titleSuffix}</h2>
