...
// --- helpers ---
const svInt = (v) => Number(v).toLocaleString("sv-SE");
...
export function TopCitiesChart({ data, mode, CHART, color }) {
  ...
  const tooltipFormatter = (v, name) =>
    mode === "customers" ? [svInt(v), "Customers"] : [`${svInt(v)} KSEK`];
...
export function AgeDistributionChart({
  ...
  // Build data rows
  const sw = ageByCountry?.["Sweden"] || {};
  const female = sw["Female"] || {};
  const male = sw["Male"] || {};
        ...
        {genderMode === "Both" ? (
          <BarChart data={data} margin={CHART.margin} barCategoryGap="20%" barGap={6}>
            ...
            <YAxis
              ...
              tickFormatter={svInt}
            />
            <Tooltip formatter={(v, name) => [svInt(v), name]} cursor={false} />
