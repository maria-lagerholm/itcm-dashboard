...
export default function Sweden() {
  ...
  // Fetch top 10 cities by unique customers for Sweden (country_id=205) on mount or when base changes
  useEffect(() => {
    fetch(`${base}/api/country/205/top-cities?limit=10`)
      ...
      .catch(e => {
        ...
        console.error("Error fetching Sweden top cities by customers:", e);
      });
  }, [base]);

  // Fetch top 10 cities by revenue for Sweden only when switching to "revenue" mode and not already loaded
  useEffect(() => {
    ...
    fetch(`${base}/api/cities_by_revenue`)
      ...
      .then(j => {
        ...
        // Sweden's data: array of { city, ksek }
        const sweden = Array.isArray(map?.["Sweden"]) ? map["Sweden"] : [];
        setRowsRevenue(sweden);
      })
      .catch(e => {
        // Log fetch or parse errors for debugging
        console.error("Error fetching Sweden top cities by revenue:", e);
      });
