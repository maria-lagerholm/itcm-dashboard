...
export default function Sweden() {
  ...
  useEffect(() => {
    fetch(`${base}/api/country/205/top-cities?limit=10`)
      ...
      .catch((e) => console.error("Error fetching Sweden top cities by customers:", e));
  }, [base]);
  ...
  useEffect(() => {
    ...
    fetch(`${base}/api/cities_by_revenue`)
      .then((r) => r.json())
      .then((j) => {
        const sweden = j?.top_cities_by_revenue_ksek?.["Sweden"];
        setRowsRevenue(Array.isArray(sweden) ? sweden : []);
      })
      .catch((e) => console.error("Error fetching Sweden top cities by revenue:", e));
  ...
  return (
    <main className={styles.main}>
      ...
      <section className={styles.sectionCard}>
        <header className={styles.headerRow}>
          <h3 className={styles.h3}>Sweden · Age distribution by gender</h3>
          <div className={styles.toggleRow}>
            <button
