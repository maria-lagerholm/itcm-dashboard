...
// --- helpers ---
const noInt = (v) => Number(v).toLocaleString("no-NO");
...
export function TopCitiesChart({ data, mode, CHART, color }) {
  ...
  const tooltipFormatter = (v, name) =>
    mode === "customers" ? [noInt(v), "Customers"] : [`${noInt(v)} KSEK`];
...
export function AgeDistributionChart({
  ...
  // Build data rows
  const no = ageByCountry?.["Norway"] || {};
  const female = no["Female"] || {};
  const male = no["Male"] || {};
        ...
        {genderMode === "Both" ? (
          <BarChart data={data} margin={CHART.margin} barCategoryGap="20%" barGap={6}>
            ...
            <YAxis
              ...
              tickFormatter={noInt}
            />
            <Tooltip formatter={(v, name) => [noInt(v), name]} cursor={false} />
