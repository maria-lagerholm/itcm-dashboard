...
import { CHANNEL_COLORS, CARD, TEXT } from "../../../theme";
import { fmtInt } from "../utils/formatters";

/**
 * Stacked horizontal bar chart for order channels.
 * @param {Array} rows - Channel metadata (order, color, count).
 * @param {Array} data - Chart data (single row, normalized values).
        ...
        <BarChart data={data} layout="vertical">
          ...
          <Tooltip
            ...
            formatter={(val, name) => {
              ...
              const label = name?.toString().replace(/\b\w/g, m => m.toUpperCase());
              return [`${pct}% (${fmtInt(count)})`, label];
            }}
          />
          <Legend
            ...
            formatter={v => (
              <span style={{ color: "#000", fontSize: 14 }}>
                {v?.toString().replace(/\b\w/g, m => m.toUpperCase())}
              </span>
            )}
          />
          <Customized
