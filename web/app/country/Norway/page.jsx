...
// --- Age Distribution Chart for Norway ---
function AgeDistributionChart({
  ...
  return (
    <div style={{ width: "100%", height: 340, border: "1px solid #eee", borderRadius: 8, padding: 12, background: "#fafafa", fontSize: 14 }}>
      {loading ? (
...
function sortByMonthJanToDec(arr) {
  // If .month is not present, return as is
  if (!Array.isArray(arr) || !arr.length || !arr[0].month) return arr;
  // Map: monthNum = parseInt(month.split("-")[1], 10)
  return [...arr].sort((a, b) => {
...
export default function Norway() {
  ...
  useEffect(() => {
    fetch(`${base}/api/country/160/top-cities?limit=10`)
      .then(r => r.json())
  ...
  useEffect(() => {
    ...
    fetch(`${base}/api/cities_by_revenue`)
      ...
      .then(j => {
        ...
        const norway = Array.isArray(map?.["Norway"]) ? map["Norway"] : [];
        setRowsRevenue(norway);
      })
      .catch(console.error);
  }, [mode, rowsRevenue.length, base]);

  // --- Fetch age distribution for Norway ---
  useEffect(() => {
    setAgeLoading(true);
  ...
  const data = useMemo(
    () => {
      ...
      // If data has .month, sort by Jan-Dec
      if (d.length && d[0].month) {
        return sortByMonthJanToDec(d);
      }
      return d;
    },
  ...
  const fmtInt = (v) => Number(v).toLocaleString("nb-NO");
  ...
  return (
    <main style={{ padding: 20, maxWidth: 1000, margin: "0 auto", fontSize: 14 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 12, fontSize: 14 }}>
        <h2 style={{ fontSize: 14 }}>Norway · Top 10 cities by {titleSuffix}</h2>
      ...
      {/* --- Age distribution for Norway --- */}
      <section style={{ marginTop: 32, marginBottom: 32, background: "#fff", borderRadius: 8, boxShadow: "0 2px 8px rgba(30,41,59,0.04)", padding: 20, fontSize: 14 }}>
        <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <h3 style={{ margin: 0, fontSize: 16, fontWeight: 600 }}>Norway · Age distribution by gender</h3>
          <div style={{ display: "flex", gap: 8 }}>
            <button
