...
export default function Norway() {
  ...
  useEffect(() => {
    fetch(`${base}/api/country/160/top-cities?limit=10`)
      ...
      .catch((e) => console.error("Error fetching Norway top cities by customers:", e));
  }, [base]);
  ...
  useEffect(() => {
    ...
    fetch(`${base}/api/cities_by_revenue`)
      .then((r) => r.json())
      .then((j) => {
        const norway = j?.top_cities_by_revenue_ksek?.["Norway"];
        setRowsRevenue(Array.isArray(norway) ? norway : []);
      })
      .catch((e) => console.error("Error fetching Norway top cities by revenue:", e));
  ...
  return (
    <main className={styles.main}>
      ...
      <section className={styles.sectionCard}>
        <header className={styles.headerRow}>
          <h2 className={styles.h2}>
            Norway · Top 10 cities by {mode === "customers" ? "unique customers" : "revenue (KSEK)"}
          </h2>
      ...
      <section className={styles.sectionCard}>
        <header className={styles.headerRow}>
          <h3 className={styles.h3}>Norway · Age distribution by gender</h3>
          <div className={styles.toggleRow}>
            <button
