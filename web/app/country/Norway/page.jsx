...
export default function Norway() {
  // State for current chart mode: "customers" or "revenue"
  ...
  // Fetch top 10 cities by unique customers for Norway (country_id=160) on mount or when base changes
  useEffect(() => {
    fetch(`${base}/api/country/160/top-cities?limit=10`)
      ...
      .catch(e => {
        ...
        console.error("Error fetching Norway top cities by customers:", e);
      });
  }, [base]);

  // Fetch top 10 cities by revenue for Norway only when switching to "revenue" mode and not already loaded
  useEffect(() => {
    ...
    fetch(`${base}/api/cities_by_revenue`)
      ...
      .then(j => {
        ...
        // Norway's data: array of { city, ksek }
        const norway = Array.isArray(map?.["Norway"]) ? map["Norway"] : [];
        setRowsRevenue(norway);
      })
      .catch(e => {
        // Log fetch or parse errors for debugging
        console.error("Error fetching Norway top cities by revenue:", e);
      });
  ...
  return (
    <main style={{ padding: 20, maxWidth: 1000, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 12 }}>
        <h2>Norway · Top 10 cities by {titleSuffix}</h2>
