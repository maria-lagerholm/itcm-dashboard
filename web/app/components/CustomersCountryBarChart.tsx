...
import { RowSegments } from "../hooks/useCustomersByCountry";
import {
  COLORS, CHART, SEGMENT_ORDER, SEGMENT_LABELS, CARD, TEXT, UI, TOOLTIP
} from "../theme";
...
function SegmentsLegend() {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 6, fontFamily: TEXT.family, color: TEXT.color }}>
      ...
      <div>
        {SEGMENT_LABELS.New} · {SEGMENT_LABELS.Returning} · {SEGMENT_LABELS.Loyal}
      </div>
    </div>
  );
}

export default function CustomersCountryBarChart({ data, totalCustomers }: Props) {
    ...
    >
      {totalCustomers != null ? (
        <div style={{ display: "flex", justifyContent: "flex-end" }}>
          <span style={{ whiteSpace: "pre" }}>Total customers: {fmt(totalCustomers)}</span>
        </div>
      ) : <div />}
      ...
      <ResponsiveContainer>
        ...
        >
          ...
          />
          <Tooltip
            content={(props) => <SegmentsTooltip {...props} />}
