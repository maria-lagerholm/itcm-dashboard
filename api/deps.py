...
def get_customers_df() -> pd.DataFrame:
    return _get("_customers_df", CUSTOMERS_PARQUET_PATH)

def get_transactions_df() -> pd.DataFrame:
    return _get("_transactions_df", TRANSACTIONS_PARQUET_PATH)

def get_articles_df() -> pd.DataFrame:
    return _get("_articles_df", ARTICLES_PARQUET_PATH)

def get_orders_df() -> pd.DataFrame:
    return _get("_orders_df", ORDERS_PARQUET_PATH)

def get_order_items_df() -> pd.DataFrame:
    return _get("_order_items_df", ORDER_ITEMS_PARQUET_PATH)

def get_customer_summary_df() -> pd.DataFrame:
    return _get("_customer_summary_df", CUSTOMER_SUMMARY_PARQUET_PATH)

def get_country_summary_df() -> pd.DataFrame:
    return _get("_country_summary_df", COUNTRY_SUMMARY_PARQUET_PATH)

def get_city_summary_df() -> pd.DataFrame:
    return _get("_city_summary_df", CITY_SUMMARY_PARQUET_PATH)

def get_city_monthly_revenue_df() -> pd.DataFrame:
    return _get("_city_monthly_revenue_df", CITY_MONTHLY_REVENUE_PARQUET_PATH)

def get_countries_by_channel_df() -> pd.DataFrame:
    return _get("_countries_by_channel_df", COUNTRIES_BY_CHANNEL_PARQUET_PATH)

def get_return_buckets_df() -> pd.DataFrame:
    return _get("_return_buckets_df", RETURN_BUCKETS_PARQUET_PATH)

def get_top_brands_df() -> pd.DataFrame:
    return _get("_top_brands_df", TOP_BRANDS_PARQUET_PATH)

def get_top_categories_df() -> pd.DataFrame:
    return _get("_top_categories_df", TOP_CATEGORIES_PARQUET_PATH)

def get_top_groups_df() -> pd.DataFrame:
    return _get("_top_groups_df", TOP_GROUPS_PARQUET_PATH)

def get_top_repurchase_df() -> pd.DataFrame:
    return _get("_top_repurchase_df", TOP_REPURCHASE_PARQUET_PATH)
