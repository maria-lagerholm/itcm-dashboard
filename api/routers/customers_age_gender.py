...
def customers_age_gender(customers: pd.DataFrame = Depends(get_customer_summary_df)):
    df = customers[["country", "age", "gender"]].dropna(subset=["country", "age", "gender"])
    ...
    by_country = {}
    for (country, gender), g in table.groupby(["country", "gender"], sort=False):
        by_country.setdefault(country, {})[gender] = {str(int(a)): int(c) for a, c in zip(g["age"], g["count"])}
